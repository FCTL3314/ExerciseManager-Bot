...
class ITokenManager(ABC):
    ...
    @abstractmethod
    async def save(self, user_id: str, token: str, token_type: TokenType) -> None: ...

    @abstractmethod
    async def get(self, user_id: str, token_type: TokenType) -> KeyValueType: ...
...
class TokenManager(ITokenManager):
    ...
    async def _generate_key(self, user_id: str, token_type: TokenType) -> str:
    ...
    async def get(self, user_id: str, token_type: TokenType) -> str:
        key = await self._generate_key(user_id, token_type)
